...
function CustomTooltip({ active, payload, label, unit, color }) {
  ...
  return (
    <div className="bg-[#111] border border-[#2a2a2a] rounded-xl px-3 py-2 text-xs shadow-2xl">
      ...
      {payload.map((entry, i) => (
        <p key={i} className="font-bold tabular-nums" style={{ color: entry.color }}>
...
export default function EnergyLineChart({
  ...
  productionLoading, consumptionLoading,
  productionError, consumptionError,
  onRetryProduction, onRetryConsumption,
  color = '#20F3C7', unit, frequency, activeTab,
}) {
  const loading = activeTab === 'production' ? productionLoading : consumptionLoading
  const error   = activeTab === 'production' ? productionError  : consumptionError
  const onRetry = activeTab === 'production' ? onRetryProduction : onRetryConsumption
  ...
  if (error) return <ErrorBlock message={error} onRetry={onRetry} label="chart data" />
