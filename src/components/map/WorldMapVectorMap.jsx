...
export default function WorldMapVectorMap(props) {
  const { data, color, unit, loading } = props;
  const [count, setCount] = useState(1); // zoom
  ...
  // Tvoja data iz API-ja (samo zemlje)
  const countryData = useMemo(() => {
    return data
      .filter((item) => item.countryRegionTypeId === "c")
      .map((item) => ({
        id: item.countryRegionId, // npr. "SRB", "BIH"...
        name: item.countryRegionName,
  ...
  // Inicijalizacija eventova na SVG path-ovima
  useEffect(() => {
    // ISO2 (iz SVG data-code) → ISO3 (iz tvog API-ja)
    const iso2ToIso3 = {
      AF: "AFG",
      AX: "ALA",
      AL: "ALB",
    ...
    const countriesGroup = svg.querySelector("#countries") || svg;
    const paths = countriesGroup.querySelectorAll("path[data-code]");

    // Compute min/max for opacity scaling
    const values = countryData.map((d) => Number(d.value)).filter(Boolean);
    ...
    // Apply data colors to all paths
    paths.forEach((path) => {
      const code = path.getAttribute("data-code");
      const lookupId = iso2ToIso3[code] || code;
      const matched = countryData.find((item) => item.id === lookupId);

      if (matched) {
        const norm =
          maxVal === minVal
            ? 1
            : (Number(matched.value) - minVal) / (maxVal - minVal);
        const opacity = 0.5 + norm * 0.3; // 0.5 → 0.8
        ...
        path.dataset.hasData = "0";
      }
    });

    const restorePath = (p) => {
      if (p.dataset.hasData === "1") {
        p.style.fill = color;
        p.style.fillOpacity = p.dataset.baseOpacity;
    ...
    const handleMouseOver = (event) => {
      ...
      if (prevPath && prevPath !== path) restorePath(prevPath);

      // Neon glow on the stroke using drop-shadow
      path.style.stroke = color;
      path.style.strokeWidth = "1px";
      path.style.strokeOpacity = "1";
      path.style.filter = `drop-shadow(0 0 4px ${color}) drop-shadow(0 0 8px ${color})`;
      prevPath = path;

      if (!matched) {
        if (refInfo.current) refInfo.current.style.display = "none";
        return;
      }

      if (refInfo.current) {
        refInfo.current.style.display = "block";
        document.getElementById("countryName").innerText = matched.name || code;
        document.getElementById("countryActivity").innerText =
          matched.activity || "";
        document.getElementById("countryPeriod").innerText =
          matched.period || "";
        const numVal = parseFloat(matched.value);
        document.getElementById("countryValue").innerText = isNaN(numVal)
          ? matched.value || ""
          : numVal.toFixed(2);
        document.getElementById("countryUnit").innerText = matched.unit || "";
      }
    };
    ...
    const moveTooltip = (event) => {
      if (!refInfo.current || refInfo.current.style.display === "none") return;
      const tooltip = refInfo.current;
    ...
    return () => {
      ...
      svg.removeEventListener("mouseleave", hideTooltip);
      document.removeEventListener("mousemove", moveTooltip);
    };
  }, [countryData, color, darkTheme]);
  ...
  const handleZoomIn = () => setCount((c) => c + 0.2);
  const handleZoomOut = () => setCount((c) => Math.max(c - 0.2, 0.4));
  const handleWheel = (e) => {
    e.preventDefault();
    setCount((c) => Math.max(c - e.deltaY * 0.001, 0.4));
  };
    ...
    <>
      ...
      >
        ...
        <div
          ref={refInfo}
          id="info-box"
          className="bg-white dark:bg-gray-900 text-black dark:text-white shadow-2xl px-5 py-4 rounded-2xl pointer-events-none z-[100] border border-gray-200 dark:border-gray-700"
          style={{ display: "none", minWidth: "220px", position: "fixed" }}
        >
          <p id="countryName" className="font-bold text-lg mb-1" />
          <p
            id="countryActivity"
          ...
          <p
            ...
            className="text-sm text-gray-600 dark:text-gray-400"
          />
          <p id="countryValue" className="text-xl font-semibold mt-2" />
          <p id="countryUnit" className="text-xs text-gray-500" />
        </div>
        ...
        <svg
          ...
          style={{
            transform: `scale(${count}) translate(${mapPosition.x}px, ${mapPosition.y}px)`,
        ...
        >
          ...
          >
            ...
            ></path>
            <path
              d="M469.4,163.99l0.42,-0.5l-0.01,-0.52l-1.15,-1.63l1.43,-0.62l1.33,0.12l1.17,1.06l0.46,1.13l1.34,0.64l0.35,1.35l1.46,0.9l0.76,-0.29l0.2,0.69l-0.48,0.78l0.22,1.12l1.05,1.22l-0.77,0.8l-0.37,1.52l-1.21,0.08l0.24,-0.64l-0.39,-0.54l-2.08,-1.64l-0.9,0.05l-0.48,0.94l-2.12,-1.37l0.53,-1.6l-1.11,-1.37l0.51,-1.1l-0.41,-0.57Z M472.77,172.64l-1.08,-1.29l0.96,-0.77l0.29,-0.83l1.98,1.64l-0.36,0.67l-1.79,0.58Z"
              data-code="RS"
              fill="#D0D5DD"
              fillOpacity="1"
              stroke="none"
